...
window.Board = (() => {
  ...
  const PRESSURE_RULES = [
    ...
    {
      id: "overdraft", label: "Cash / Overdraft",
      evaluate() {
        var cash = GameState.balance.cash;
        if (cash >= 0) return null;
        var overdraft = Math.abs(cash);
        if (overdraft > 50)  return { points: 3, reason: "DEFAULT RISK: Overdraft at $" + fmt(overdraft, 1) + "M — lenders may call loans" };
        if (overdraft > 30)  return { points: 3, reason: "Critical overdraft of $" + fmt(overdraft, 1) + "M — immediate action required" };
        if (overdraft > 15)  return { points: 2, reason: "Overdraft at $" + fmt(overdraft, 1) + "M — credit facilities under pressure" };
        if (overdraft > 5)   return { points: 1, reason: "Overdraft of $" + fmt(overdraft, 1) + "M — liquidity warning" };
        return { points: 0, reason: "Minor overdraft of $" + fmt(overdraft, 1) + "M — monitor closely" };
      },
    },
    {
      id: "minimum_payout", label: "Minimum Payout",
      evaluate() {
        // Only enforce from Year 2 onwards — Year 1 is orientation
        if (GameState.meta.year < 2) return null;
        // Only enforce if AFFO is meaningfully positive
        var affo = GameState.pnl.affo || 0;
        var divPaid = GameState.pnl.dividendsPaid || 0;
        if (affo < 1.0) return null;
        var payoutRatio = divPaid / affo;
        if (payoutRatio < 0.30) return { points: 3, reason: "Payout ratio critically low at " + fmt(payoutRatio * 100, 0) + "% — board expects minimum 50% of AFFO distributed" };
        if (payoutRatio < 0.50) return { points: 1, reason: "Payout ratio below 50% threshold: " + fmt(payoutRatio * 100, 0) + "% — board expects REIT to distribute earnings" };
        if (payoutRatio >= 0.80) return { relief: 1, reason: "Strong payout ratio of " + fmt(payoutRatio * 100, 0) + "%" };
        return null;
      },
    },
    {
