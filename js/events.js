...
window.Events = (() => {
  ...
  function rollEvents() {
    ...
    const eventSlots = Math.min(4, 2 + Math.floor(year / 4));
    ...
    // Build probability-adjusted list. Harmful events escalate with the years
    // (the world gets more dangerous); positive events do not.
    const candidates = EVENT_CATALOGUE.map(evt => {
      const bias = evt.cycleBias[cycle] || 1.0;
      evt.harmful = !!HARMFUL_IDS[evt.id];
      // Harmful events ramp hard after Year 5; benign ones stay flat.
      var yearMod;
      if (evt.harmful) {
        yearMod = 1 + Math.max(0, year - 5) * 0.12;   // +12%/yr of danger after Y5
      } else {
        yearMod = 1;
      }
      return {
        evt,
        probability: Math.min(0.70, evt.baseProbability * bias * yearMod),
      };
    });

    // Roll harmful events FIRST (so positive events don't eat the slots), then
    // benign ones. A short global cooldown keeps it from being every single
    // quarter, but the world should bite regularly — especially late-game.
    const firedIds = new Set();
    var negativeCooldown = GameState._negEventCooldown || 0;
    var firedNegativeThisQuarter = 0;
    var maxNegPerQuarter = year >= 8 ? 2 : 1;  // late game can stack two shocks

    // Harmful pass
    candidates.filter(c => c.evt.harmful).forEach(({ evt, probability }) => {
      if (firedIds.has(evt.id)) return;
      if (year <= 1) return;                                   // Year 1 shielded
      if (negativeCooldown > 0) return;                        // global breather
      if (firedNegativeThisQuarter >= maxNegPerQuarter) return;
      if (Math.random() < probability) {
        ...
        if (result) {
          ...
          // Shorter cooldown (1-2 quarters) so setbacks come regularly.
          GameState._negEventCooldown = 1 + Math.floor(Math.random() * 2);
        }
      }
    });
